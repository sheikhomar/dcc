...
def compute_cluster_pick_proba(model, image_paths, n_clusters):
    ...
    kmeans_labels = km.fit_predict(feature_map_pca)

    proba = np.bincount(kmeans_labels) / np.bincount(kmeans_labels).sum()
    proba_inv = 1 / proba
    label_proba = proba_inv / proba_inv.sum()
    return kmeans_labels, label_proba
...
def generate_augmented_images(source_dir: Path, target_dir: Path, class_name: str, n_target: int, quantize: bool):
    ...
    experiment_dir = source_dir.parent.parent
    model_path = experiment_dir / "checkpoints" / "imagenet-finetuned"
    ...
    with tqdm(total=n_target) as pbar:
        ...
        cluster_labels, pick_proba = compute_cluster_pick_proba(model, image_paths, 70)

        i = 0
        
        while i + len(image_paths) < n_target:
            picked_cluster = np.random.choice(np.arange(pick_proba.shape[0]), p=pick_proba)
            filtered_image_paths = np.array(image_paths)[cluster_labels == picked_cluster]
            image_path = np.random.choice(filtered_image_paths)
